...
class GroupsParser {
  ...
  List<String> getFacultiesLinks(http.Response response) {
    ...
    var facultiesElements = facultiesHtmlDocument
        .getElementsByClassName('faculty-list__link');
    var facultiesCount = facultiesElements.length;
    var facultiesLinks = List.filled(facultiesCount, ' ');

    var progressBar =
        ProgressBar(totalLength: facultiesCount);

    for (var i = 0; i < facultiesCount; i++) {
      progressBar.update(i + 1);

      var sublink =
          facultiesElements[i].attributes['href']!;
  ...
  Future<List<String>> getScheduleLinksForUpcomingWeeks(
    ...
    for (var i = 0; i < numberOfUpcomingWeeks; ++i) {
      var nextWeekDate =
          todaysDate.add(Duration(days: 7 * i));
  ...
  Future<List<String>> findProfessors(String url) async {
    final response =
        await httpClientService.get(Uri.parse(url));
    var weekScheduleHtmlDocument = parse(response.body);
    var weekProfessors = weekScheduleHtmlDocument
        .getElementsByClassName('lesson__teachers');

    var professors = List.filled(weekProfessors.length, '');

    for (var i = 0; i < weekProfessors.length; ++i) {
      professors[i] =
          normalizeProfessorNames(weekProfessors[i].text);
    }
  ...
  Future<List<String>> fetchAllProfessors(
    ...
    var scheduleLinksForUpcomingWeeks =
        (await getScheduleLinksForUpcomingWeeks(
                numberOfUpcomingWeeks))
            .map((link) => '$url?date=$link')
            .toList();

    var professors = <String>[];
    for (final link in scheduleLinksForUpcomingWeeks) {
      var partOfFoundedProfessors =
          await findProfessors(link);
      professors.addAll(partOfFoundedProfessors);
    }
    return professors.toSet().toList();
  }

  Future<String> findMisteryProfessor(
      String misteryProfessor,
      List<Map<String, String>>
          professorsIdsAndNames) async {
    var professorsNames =
        professorsIdsAndNames.map((data) => data['name']!);
    for (final partOfMisteryProfessor
        in misteryProfessor.split(' ')) {
      if (professorsNames
          .contains(partOfMisteryProfessor)) {
        var id = professorsIdsAndNames.firstWhere((data) =>
            data['name']! == partOfMisteryProfessor)['id'];
        return id!;
      }
    }
    return 'Not found';
  ...
  Future<List<String>> matchNameWithId(
      List<String> professors) async {
    ...
    for (final professor in professors) {
      ...
      if (professorsNames.contains(professor)) {
        var id = professorsIdsAndNames.firstWhere(
            (data) => data['name']! == professor)['id'];
        professorsId.add(id!);
      } else {
        var result = await findMisteryProfessor(
            professor, professorsIdsAndNames);
        if (result != 'Not found') {
          ...
          databaseService.professorsData =
              await provider.getOnceAllProfessors();
          professorsIdsAndNames = await databaseService
              .getProfessorsIdsAndNames();
        }
      }
    }
  ...
  Future<void> addProfessorIfHeNotExists(
  ...
  Future<void> parseGroupSchedulePage(
      String groupLink, String number) async {
    ...
    for (final professorId in professorsIds) {
      ...
      await databaseService.addProfessorToGroup(

...
class DatabaseProviderImpl implements DatabaseProvider {
  ...
  Future<List<Review>> getAllReviewByUser(int userId) =>
      (database.select(database.reviews)..where((u) => u.userId.equals(userId))).get();

  @override
  Stream<List<Review>> getAllReviewsByProfessor(String professorId) =>
      (database.select(database.reviews)..where((u) => u.professorId.equals(professorId))).watch();
  ...
  Stream<List<ReviewWithProfessor>> getReviewsWithProfessor(int userId) =>
      (database.select(database.reviews)..where((u) => u.userId.equals(userId)))
          .join([innerJoin(database.professors, database.professors.id.equalsExp(database.reviews.professorId))])
          .watch()
          .map(
            (rows) => rows
                .map(
